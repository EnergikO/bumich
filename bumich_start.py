...
@client.command()
# Пишет в чат набор из случайных чисел, нет ограничений, ввод диапазона чисел в любом порядке
async def roll(ctx, roll_first_number=100, roll_second_number=1, count=1):
    # Создаём видимость работы
    await ctx.trigger_typing()

    # Проверка, ввели ли числа или нет
    if isinstance(roll_first_number, int) and isinstance(roll_second_number, int) and isinstance(count, int):
        ...
        if count <= 0:
            ...
            if count == 1:
                ...
                temp_str = f'Случайные числа от {roll_first_number} до {roll_second_number}:\n'
        ...
        await ctx.send('Вы ввели не числа')
